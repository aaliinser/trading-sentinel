...
# ==========================================
# CONFIGURATION FOR 15-MINUTE TIMEFRAME
# ==========================================
SYMBOLS = ["USDJPY=X", "EURAUD=X", "USDCHF=X", "EURCAD=X", "CADJPY=X"]
TIMEFRAME = "15m"       # Changed to 15 Minutes
PERIOD_LIMIT = "60d"    # Max available for intraday data in yfinance is usually 60 days
STAKE = 6.0
PAYOUT_RATE = 0.90
BREAK_EVEN_WR = 0.5263

# The Winning Strategies from H1 Test (We test if they hold up on M15)
WINNING_STRATEGIES = [
...
def get_data(symbol, period=PERIOD_LIMIT, interval=TIMEFRAME):
    try:
        ...
        # Note: For 15m, max period is typically 60d. 
        ...
        # Check sufficient length for indicators (EMA200 needs 200 bars)
        if len(df) < 250:
             print(f"Insufficient data for {symbol} ({len(df)} bars). Need >250.")
             return None
...
def validate_strategy(strategy_cfg, symbol_df):
    ...
    min_idx = max(params['k'], params['d'], 200) + 1 # Ensure we have enough data for all indicators
    ...
    for i in range(min_idx, n-1):
        ...
        if np.isnan(k_val) or np.isnan(d_val) or np.isnan(pk) or np.isnan(pd): continue
...
def main():
    ...
    if not symbol_data:
        print("No valid data found across symbols. Exiting."); return

    final_report = []

    for strat in WINNING_STRATEGIES:
        print(f"\n>>> Validating: {strat['name']} on M15")
        all_trades_h1 = [] 
        all_trades_h2 = [] 
    ...
    if winners_found:
        print("\n🎉 SUCCESS! Some strategies held up on M15.")
    else:
        print("\n⚠️ FAILURE: No strategies passed the strict robustness test on M15.")
        print("Conclusion: The edge exists primarily on Higher Timeframes (H1/H4). Avoid M15 with this logic.")
