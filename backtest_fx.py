...
def indicators_arr(df):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        rsi = (100 - (100 / (1 + ratio))).fillna(50).to_numpy(dtype=float)
...
def build_report():
    ...
    for sym in SYMBOLS:
        ...
        for W, lbl in WINDOWS:
            ...
            for i in range(i0, n):
                ...
                if not np.isnan(dr) and dr > a["maxdr"]:
                    a["maxdr"] = dr
                if d1 != d2:
                    a["flips"] += 1
    ...
    msg = "🧪 *اختبار تكافؤ نافذة البيانات*\n(5 أزواج × 3 أيام فحص × 4 نوافذ)\n\n"
    msg += "*جزء A — تطابق القرارات مع تاريخ كامل:*\n"
    msg += "```\n"
    ...
    msg += "*جزء B — شموع متاحة فجر الاثنين (بعد عطلة):*\n"
    msg += "```\n"
